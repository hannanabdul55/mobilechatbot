...
all = []
feature_list= ["bluetooth","wifi","front","camera","cameras","back","screen","hd","processor","accelerometer","barometer","gyro"]
all.extend(phones)
all.extend(org_list1)
all.extend(os_list)
all.extend(currency_symbols)
all.extend(size_list)
all.extend(feature_list)
all.extend(int_list)
...
class FeatureFunctions1(object):
    ...
    def fprice_query2(self, h, rel):
        ...
        if rel.lower() == "price_query":
            for i in h["updates"]:
                if str(i["word"]).lower() in currency_symbols or p1.match(i["word"].lower()) or p2.match(i["word"].lower()):
                    for j in h["updates"]:
                        if j["tag"] == "Feature":
                            return 0						
                    return 1
